...
void test_tiny_stories(MockIC &mockIC) {
  ...
  for (const auto &model : models) {
    ...
    mockIC.run_test(
        test_name, load_model, candid_in,
        candid_out,
    ...
    // Check readiness
    // '()' -> '(variant { Ok = record { status_code = 200 : nat16; } })'
    mockIC.run_test(std::string(__func__)+": "+"ready OK - " + model, ready, "4449444c0000",
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      // -> ...
      mockIC.run_test(
          std::string(__func__)+": "+"run_update for chat " + std::to_string(i) + " - " + model,
          run_update,
      ...
      // -> ...
      mockIC.run_test(
          std::string(__func__)+": "+"run_update for chat " + std::to_string(i) + " continued - " + model,
          run_update,
